...
export default {
  ...
  desc: _('AgentDock 版本和项目信息', 'AgentDock version and project info', 'AgentDock 版本和項目資訊', 'AgentDock バージョンとプロジェクト情報', 'AgentDock 버전 및 프로젝트 정보', 'Thông tin phiên bản và dự án AgentDock', 'Información de versión y proyecto de AgentDock', 'Informações de versão e projeto do AgentDock', 'Информация о версии и проекте AgentDock', 'Informations de version et de projet AgentDock', 'AgentDock-Version und Projektinformationen'),
  ...
  techStack: _('AgentDock 基于 Tauri v2 构建，前端 Vanilla JS + Vite，后端 Rust。', 'AgentDock is built with Tauri v2, Vanilla JS + Vite frontend, Rust backend.', 'AgentDock 基於 Tauri v2 構建，前端 Vanilla JS + Vite，後端 Rust。', 'AgentDock は Tauri v2 で構築、フロントエンド Vanilla JS + Vite、バックエンド Rust。'),
  copyright: _('MIT License © 2026 ', 'MIT License © 2026 ', 'MIT License © 2026 '),
  ...
  versionAvailable: _('AgentDock v{version} 可用', 'AgentDock v{version} available', 'AgentDock v{version} 可用', 'AgentDock v{version} が利用可能です', 'AgentDock v{version} 사용 가능', 'AgentDock v{version} đã có sẵn'),
  ...
  updateCommandHint: _('如果 git pull 失败，可先执行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 丢弃本地修改。<br>路径请替换为实际的 AgentDock 安装目录。', 'If <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git pull</code> fails, run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> first to discard local changes.<br>Replace the path with your actual AgentDock install directory.', '如果 git pull 失敗，可先執行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 捨棄本地修改。<br>路徑請替換為實際的 AgentDock 安裝目錄。', 'git pull に失敗した場合は、先に <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> を実行してローカル変更を破棄してください。<br>パスは実際の AgentDock インストールディレクトリに置き換えてください。', 'git pull 이 실패하면 먼저 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 를 실행해 로컬 변경을 버리세요.<br>경로는 실제 AgentDock 설치 디렉터리로 바꿔 주세요.', 'Nếu git pull thất bại, hãy chạy <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> trước để bỏ các thay đổi cục bộ.<br>Hãy thay đường dẫn bằng thư mục cài đặt AgentDock thực tế của bạn.'),
  ...
  projectAgentDock: _('OpenClaw 可视化管理面板，Tauri v2 桌面应用', 'OpenClaw visual management panel, Tauri v2 desktop app', 'OpenClaw 可视化管理面板，Tauri v2 桌面應用'),
  ...
  contributeDesc: _('AgentDock 是开源项目，欢迎参与贡献！遇到问题请提 Issue，功能建议和代码改进欢迎提 PR。', 'AgentDock is open source! Report bugs via Issues, submit feature requests and code improvements via PRs.', 'AgentDock 是開源項目，欢迎參與貢獻！遇到問題請提 Issue，功能建議和代碼改進欢迎提 PR。', 'AgentDock はオープンソースプロジェクトです。Issue や PR を歓迎します！', 'AgentDock은 오픈소스 프로젝트입니다. Issue와 PR을 환영합니다!'),
  ...
  domesticMirrorHint: _('AgentDock 国内镜像将在正式发布入口确认后提供。', 'AgentDock China mirror will be provided after the production release entry is confirmed.', 'AgentDock 國內鏡像將在正式發布入口確認後提供。'),
  companyName: _('', '', ''),
  ...
  companyDesc: _('我们是 OpenClaw 汉化版（3000+ Star）和 AgentDock 的作者团队。日常做 AI Agent 相关的产品和开源工具，也接企业私有化部署、定制开发之类的活儿。有事直接群里找我们就行。', 'We are the team behind OpenClaw Chinese Translation (3000+ Stars) and AgentDock. We build AI Agent products and open source tools, and offer enterprise deployment and custom development services.', '我們是 OpenClaw 漢化版（3000+ Star）和 AgentDock 的作者團队。日常做 AI Agent 相關的產品和開源工具，也接企業私有化部署、定制開發之類的活儿。有事直接群里找我們就行。'),
