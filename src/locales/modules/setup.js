...
export default {
  ...
  headerTitle: _('欢迎使用 AgentDock', 'Welcome to AgentDock', '', 'AgentDock へようこそ', 'AgentDock에 오신 것을 환영합니다', 'Chào mừng đến AgentDock', 'Bienvenido a AgentDock', 'Bem-vindo ao AgentDock', 'Добро пожаловать в AgentDock', 'Bienvenue sur AgentDock', 'Willkommen bei AgentDock'),
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 AgentDock 后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing AgentDock and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉 AgentDock 后從終端啟動：'),
  ...
  aiAssistant: _('DeepAi助手', 'DeepAi Assistant', 'DeepAi助手', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant', 'DeepAi Assistant'),
  ...
  installHint: _('点击安装后，将默认安装当前 AgentDock 版本绑定的推荐稳定版；如需升降级，可稍后到「关于」页面切换版本。', 'Installation will use the recommended stable version bound to this AgentDock release. To change versions later, go to the About page.', '点擊安裝后，將預設安裝目前 AgentDock 版本綁定的推薦穩定版；如需升降級，可稍后到「關於」頁面切換版本。'),
  ...
  methodHintR2: _('从DeepAi助手 CDN 下载独立安装包，自带 Node.js 运行时，无需 npm。国内下载速度最快。', 'Downloads standalone package from CDN with bundled Node.js runtime, no npm needed. Fastest for China.', '從DeepAi助手 CDN 下載獨立安裝包，自帶 Node.js 執行時，無需 npm。國內下載速度最快。'),
  ...
  envHintDesc: _('AgentDock 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 OpenClaw。若你的 OpenClaw 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', 'AgentDock desktop can only directly manage OpenClaw installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your OpenClaw actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', 'AgentDock 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 OpenClaw。若你的 OpenClaw 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 AgentDock Web 版', 'Advanced alternative: use AgentDock Web inside that environment', '進階替代方案：在對應環境裡使用 AgentDock Web 版'),
  ...
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/DeepAi助手/openclaw 或 1186258278/openclaw', 'e.g. ghcr.io/DeepAi助手/openclaw or 1186258278/openclaw', '例如 ghcr.io/DeepAi助手/openclaw 或 1186258278/openclaw'),
  ...
  wslWebDesc: _('如果你的 OpenClaw 本来就运行在 WSL 里，可打开 WSL 终端部署 AgentDock Web 版：', 'If your OpenClaw already runs inside WSL, open a WSL terminal and deploy AgentDock Web there:', '如果你的 OpenClaw 本來就運行在 WSL 裡，可開啟 WSL 終端部署 AgentDock Web 版：'),
  ...
  dockerDesc: _('如果你的 OpenClaw 本来就运行在容器里，可在容器内安装 OpenClaw + AgentDock Web 版：', 'If your OpenClaw already runs inside a container, install OpenClaw + AgentDock Web there:', '如果你的 OpenClaw 本來就運行在容器裡，可在容器內安裝 OpenClaw + AgentDock Web 版：'),
  ...
  agentdockDeployPending: _('AgentDock Web 一键部署脚本将在正式发布入口确认后提供；当前请先使用桌面端，或手动安装 OpenClaw 后绑定 CLI 路径。', 'AgentDock Web one-command deployment will be provided after the production release entry is confirmed. For now, use the desktop app or install OpenClaw manually and bind the CLI path.', 'AgentDock Web 一鍵部署腳本將在正式發布入口確認後提供；目前請先使用桌面端，或手動安裝 OpenClaw 後綁定 CLI 路徑。'),
