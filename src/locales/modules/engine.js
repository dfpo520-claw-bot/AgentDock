...
export default {
  ...
  choiceSubtitle: _('AgentDock 支持 OpenClaw 与 Hermes Agent。你可以先安装其中一个，也可以稍后再配置。', 'AgentDock supports OpenClaw and Hermes Agent. Install one now, enable both, or configure later.', 'AgentDock 支援 OpenClaw 與 Hermes Agent。你可以先安裝其中一個，也可以稍後再配置。'),
  choiceNoteTitle: _('之后仍可切换', 'You can switch later', '之後仍可切換'),
  choiceNoteDesc: _('该选择会保存到 agentdock.json。安装完成后，可通过侧边栏的引擎切换器切换到其他引擎。', 'This choice is saved to agentdock.json. After setup, use the engine switcher in the sidebar to switch engines.', '該選擇會儲存到 agentdock.json。安裝完成後，可透過側邊欄的引擎切換器切換到其他引擎。'),
  choiceSaved: _('已保存引擎选择', 'Engine choice saved', '已儲存引擎選擇'),
  choiceSaveFailed: _('保存引擎选择失败', 'Failed to save engine choice', '儲存引擎選擇失敗'),
  choiceStatusInstalled: _('已安装', 'Installed', '已安裝'),
  choiceStatusInstalledVersion: _('已安装 {version}', 'Installed {version}', '已安裝 {version}'),
  choiceStatusMissing: _('未安装', 'Not installed', '未安裝'),
  choiceStatusError: _('检测失败', 'Check failed', '檢測失敗'),
  ...
  dashNativePanelWindowsDesc: _('当前环境暂不支持直接启动 Hermes Dashboard。请继续使用 AgentDock 内置面板，或在兼容环境中运行。',
    'This environment cannot start Hermes Dashboard directly. Continue using the built-in AgentDock panel, or run it in a compatible environment.',
    '目前環境暫不支援直接啟動 Hermes Dashboard。請繼續使用 AgentDock 內建面板，或在相容環境中執行。',
    '現在の環境では Hermes Dashboard を直接起動できません。AgentDock 内蔵パネルを使うか、互換環境で実行してください。',
    '현재 환경에서는 Hermes Dashboard를 직접 시작할 수 없습니다. AgentDock 내장 패널을 계속 사용하거나 호환 환경에서 실행하세요.'),
  dashNativePanelWindowsAlt1: _('继续使用 AgentDock 内置的 Hermes 面板（推荐，体验更好）',
    'Keep using AgentDock\'s built-in Hermes panel (recommended, better experience)',
    '繼續使用 AgentDock 內建的 Hermes 面板（推薦，體驗更好）',
    'AgentDock 内蔵の Hermes パネルを使い続ける（推奨、より良い体験）',
    'AgentDock 내장 Hermes 패널을 계속 사용 (권장, 더 나은 경험)'),
  ...
  hermesGatewayForeign: _('端口已被外部进程占用（非 AgentDock spawn）', 'Port owned by external process (not spawned by AgentDock)', '埠號已被外部行程佔用'),
  hermesGatewayForeignTip: _('只能停止 AgentDock spawn 的实例', 'Can only stop instances spawned by AgentDock', '只能停止 AgentDock spawn 的實例'),
  ...
  xtFeatAgentDesc: _('预置心甜 + DeepAi助手两个助手，独立人设与记忆，可随时自定义。', 'Bundled Xintian & DeepAi助手 assistants, each with its own persona and memory — fully customizable.', '預置心甜 + DeepAi助手兩個助手，獨立人設與記憶，可隨時自定義。'),
