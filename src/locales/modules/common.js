...
export default {
  ...
  backendDownDesc: _('AgentDock 后端服务未运行，无法获取真实数据。', 'AgentDock backend is not running, so live data is unavailable.', 'AgentDock 後端服務未執行，無法取得真實資料。', 'AgentDock バックエンドが実行されていないため、実データを取得できません。', 'AgentDock 백엔드가 실행 중이 아니어서 실제 데이터를 가져올 수 없습니다.', 'Dịch vụ backend của AgentDock chưa chạy nên không thể lấy dữ liệu thực.'),
  ...
  pageLoadFailedHint: _('如果问题持续出现，请尝试重新安装 AgentDock，或在 GitHub Issues 反馈。', 'If the problem persists, try reinstalling AgentDock or report it on GitHub Issues.', '如果問題持續出現，請嘗試重新安裝 AgentDock，或到 GitHub Issues 回報。', '問題が続く場合は AgentDock を再インストールするか、GitHub Issues で報告してください。', '문제가 계속되면 AgentDock을 다시 설치하거나 GitHub Issues에 제보하세요.', 'Nếu lỗi vẫn lặp lại, hãy thử cài lại AgentDock hoặc báo trên GitHub Issues.'),
