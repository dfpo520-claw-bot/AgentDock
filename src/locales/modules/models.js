...
export default {
  ...
  qtcoolName: _('DeepAi助手', 'DeepAi助手', 'DeepAiDeepAi'),
  qtcoolRecommend: _('推荐', 'Recommended', '推薦'),
  qtcoolDesc: _('DeepAi助手模型服务，支持通过用户后台获取 API Key。', 'DeepAi assistant model service. Get your API Key from the user console.', 'DeepAi助手模型服務，支援透過使用者後台取得 API Key。'),
  qtcoolMore: _('了解更多 →', 'Learn more →'),
  qtcoolCheckin: _('获取 API Key', 'Get API Key', '取得 API Key'),
  qtcoolKeyPlaceholder: _('粘贴 API Key（在用户后台获取）', 'Paste API Key (get it from the user console)', '貼上 API Key（在使用者後台取得）'),
  ...
  qtcoolCheckinPage: _('API Key 页面', 'API key page', 'API Key 頁面'),
  qtcoolCheckinHint: _('在', 'copy your Key from', '在'),
  qtcoolDashboardHint: _('API Key 可在', 'Get your API Key from', 'API Key 可在'),
  ...
  qtcoolSelectHint: _('从DeepAi助手获取到 {count} 个可用模型，勾选需要的模型后点击添加。', 'Fetched {count} available models from DeepAi助手. Select and add.', '從DeepAiDeepAi取得到 {count} 個可用模型，勾選需要的模型后点擊新增。'),
  qtcoolKeyLabel: _('API Key', 'API Key'),
  qtcoolKeyCheckinLink: _('获取 API Key →', 'Get API Key →', '取得 API Key →'),
  qtcoolKeyPlaceholder2: _('粘贴你的 API Key', 'Paste your API Key', '貼上你的 API Key'),
  qtcoolNoKeyWarn: _('请输入 API Key（可在用户后台获取）', 'Please enter an API Key (get it from the user console)', '請輸入 API Key（可在使用者後台取得）'),
  ...
  qtcoolProviderAdded: _('已添加DeepAi助手（{count} 个模型）', 'Added DeepAi助手 ({count} models)', '已新增DeepAiDeepAi（{count} 個模型）'),
  already: _('已有', 'Exists'),
  ...
  noProvider: _('暂无服务商，请先通过上方 DeepAi助手 获取模型列表。', 'No providers yet. Use the DeepAi model list action above to add models.', '暫無服務商，請先透過上方 DeepAi助手取得模型列表。'),
