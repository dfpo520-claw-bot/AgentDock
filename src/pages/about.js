...
/**
 * Xintian mode about page: show this app version and product cards.
...
async function loadXintianData(page) {
  ...
  cards.innerHTML = `
    <div class="stat-card">
      <div class="stat-card-header"><span class="stat-card-label">${PRODUCT_IDENTITY.name}</span></div>
...
async function loadData(page) {
  ...
  try {
    ...
    // Try to read this app version from the Tauri API; fall back when unavailable.
    const panelVersion = typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '0.1.0'
  ...
  try {
    ...
    if (isFakeUpdate) {
      meta.innerHTML = `
        ...
        <a class="btn btn-primary btn-sm" href="${PRODUCT_IDENTITY.homepage}" target="_blank" rel="noopener" style="${btnSm}">${t('about.downloadFullInstaller')}</a>
        <a class="btn btn-secondary btn-sm" href="${PRODUCT_IDENTITY.releaseUrl}" target="_blank" rel="noopener" style="${btnSm}">${RELEASES_LABEL}</a>
      `
    ...
    if (!meta) return
    if (isFakeUpdate) {
      meta.innerHTML = `<span style="color:var(--warning)">Warning: ${t('about.versionMismatch', { frontend: panelVersion, binary: binaryVersion })}</span> <a class="btn btn-primary btn-sm" href="${PRODUCT_IDENTITY.homepage}" target="_blank" rel="noopener" style="${btnSm}">${t('about.downloadFullInstaller')}</a>`
    } else {
      meta.innerHTML = `<span style="color:var(--text-tertiary)">${t('about.checkUpdateFailed')}</span> <a class="btn btn-secondary btn-sm" href="${PRODUCT_IDENTITY.homepage}" target="_blank" rel="noopener" style="${btnSm}">${t('about.goToWebsite')}</a>`
    }
  }
}
  ...
  el.innerHTML = PROJECTS.map(p => `
    <div class="service-card">
      ...
      <div class="service-actions">
        <a class="btn btn-secondary btn-sm" href="${p.url}" target="_blank" rel="noopener">${p.urlLabel}</a>
...
const LINKS = [
  { label: t('about.linkWebsite'), url: PRODUCT_IDENTITY.homepage, primary: true },
...
function renderContribute(page) {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px">
      <a class="btn btn-primary btn-sm" href="${PRODUCT_IDENTITY.supportUrl}" target="_blank" rel="noopener">${SUPPORT_LABEL}</a>
      <a class="btn btn-secondary btn-sm" href="${PRODUCT_IDENTITY.repositoryUrl}" target="_blank" rel="noopener">${SOURCE_LABEL}</a>
      <a class="btn btn-secondary btn-sm" href="${PRODUCT_IDENTITY.releaseUrl}" target="_blank" rel="noopener">${RELEASES_LABEL}</a>
    </div>
  `
}
  ...
  el.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:12px">
      <div style="display:flex;align-items:center;gap:12px">
        <img src="/images/logo-brand.png" alt="${PRODUCT_IDENTITY.name}" style="width:40px;height:40px;border-radius:10px;flex-shrink:0">
        <div>
          <div style="font-weight:700;font-size:var(--font-size-md)">${PRODUCT_IDENTITY.name}</div>
          <div style="font-size:var(--font-size-sm);color:var(--text-secondary)">${PRODUCT_IDENTITY.tagline}</div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;font-size:var(--font-size-sm)">
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
          ...
          <a href="${PRODUCT_IDENTITY.homepage}" target="_blank" rel="noopener" style="color:var(--accent)">${PRODUCT_IDENTITY.homepageHost}</a>
        </div>
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
        ...
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
          ...
          <a href="${PRODUCT_IDENTITY.repositoryUrl}" target="_blank" rel="noopener" style="color:var(--accent)">${PRODUCT_IDENTITY.repositoryUrl}</a>
        </div>
        <div style="padding:12px;border-radius:var(--radius-md);border:1px solid var(--border-primary);background:var(--bg-secondary)">
          <div style="color:var(--text-tertiary);font-size:var(--font-size-xs);margin-bottom:4px">${t('about.businessCoop')}</div>
          <a href="${PRODUCT_IDENTITY.supportUrl}" target="_blank" rel="noopener" style="color:var(--accent)">${PRODUCT_IDENTITY.supportUrl}</a>
        </div>
      </div>
