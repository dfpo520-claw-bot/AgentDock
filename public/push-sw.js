/**
 * AgentDock Web Push Service Worker
 *
 * 接收来自 OpenClaw 内核（通过 web-push 协议）的推送，
 * 调 showNotification 弹出系统级通知（即使 AgentDock 已关闭）。
 *
 * 点通知 → 尝试聚焦已打开的 AgentDock 标签；都没开就打开一个新窗口。
 */
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientsList) => {
      // 已有 AgentDock 标签 → 聚焦 + 跳到 targetUrl
