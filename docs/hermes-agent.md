...
Hermes Agent 是 AgentDock 支持的第二个 AI Agent 引擎。它把会话、长期记忆、人格档案、工具调用和消息渠道放在同一个管理面板中，让 Agent 从一次性的聊天窗口，升级为可以持续运营和沉淀上下文的智能体系统。
...
AgentDock 采用多引擎架构：OpenClaw 适合已有 OpenClaw 生态用户的 Agent 管理和 Gateway 运维；Hermes Agent 则强化会话、记忆、人格和工具执行的长期运营体验。两者可以在同一个面板中统一管理。
