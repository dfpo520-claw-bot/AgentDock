...
echo -e "  ${MAGENTA}AgentDock 构建工具${RESET}"
echo -e "  ${GRAY}─────────────────────────────────────${RESET}"
if [[ "$OS" == "Darwin" ]]; then
  if [[ "$ARCH" == "arm64" ]]; then
...
if [ "$DEBUG" = true ]; then
  if [[ -n "$TARGET" ]]; then
    echo -e "  可执行文件: src-tauri/target/$TARGET/debug/agentdock"
  else
    echo -e "  可执行文件: src-tauri/target/debug/agentdock"
  fi
else
  if [[ -n "$TARGET" ]]; then
